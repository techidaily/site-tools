...
class LastPublishChecker {
  ...
  check() {
    ...
    const minHours = process.env.MIN_PUBLISH_HOURS || 24 * 20; // 根据需要调整
...
const updatePostsTask = () => {
  ...
  const addAlsoReadContent = (url, content) => {
    ...
    const shuffleMap = urlMapHelper.getShuffleUrl2TitleMap(url, randomIntFromInterval(15, 50));
    console.log(`===> 为 ${url} 添加 ${Object.keys(shuffleMap).length} 条相关文章`);
...
const publishTask = () => {
  ...
  try {
    ...
    try {
      ...
      console.log(`执行 git config --global http.postBuffer 1048576000  ${nowDate()}`);
      execSync(`git config --global http.postBuffer 1048576000`, { cwd: __dirname });
      execSync(`git config --global https.postBuffer 1048576000`, { cwd: `${__dirname}/.deploy_git` });
    } catch (e) { }
    ...
    const out = spawnSync('yarn', ['run', 'publish'], { cwd: __dirname, maxBuffer: 100 * 1024 * 1024, env: { ...process.env, ...globalEnv } });
...
const backupGit = () => {
  ...
  try {
    ...
    try {
      execSync(`git commit -m "auto backup on ${now.toISOString()}"`, { cwd: __dirname, maxBuffer: 100 * 1024 * 1024 });
    } catch (e) {
      console.error(e);
    }
...
if (gPublishHelper.check()) {
  ...
  console.log(`解析命令行, 执行指定任务. ${nowDate()}`);
  // 解析命令行参数
  if (isProcessArgsContains('--only-map')) {
    updatePostsTask();
  }
